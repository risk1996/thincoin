...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    lastBlock = firstChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Mine 667 more blocks at the old time, and check that CBV isn't setting the bit yet.
    for (int i=1; i<chainParams->GetConsensus().nMinerConfirmationWindow-4; i++) {
        lastBlock = firstChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime = nStartTime;
    for (int i=chainParams->GetConsensus().nMinerConfirmationWindow-4; i<=chainParams->GetConsensus().nMinerConfirmationWindow; i++) {
        lastBlock = firstChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }

    // Advance to the next period and transition to STARTED,
    lastBlock = firstChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    ...
    nTime += 600;
    int blocksToMine = chainParams->GetConsensus().nMinerConfirmationWindow*2; // test blocks for up to 2 time periods
    int nHeight = chainParams->GetConsensus().nMinerConfirmationWindow*3;
    ...
    for (int i=0; i<chainParams->GetConsensus().nMinerConfirmationWindow-1; i++) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
        nHeight += 1;
    }
    ...
    // next period.
    lastBlock = secondChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);

    // Mine another period worth of blocks, signaling the new bit.
    lastBlock = secondChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow*2, nTime, VERSIONBITS_TOP_BITS | (1<<bit)).Tip();
    ...
    lastBlock = secondChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow*3-1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    lastBlock = secondChain.Mine(chainParams->GetConsensus().nMinerConfirmationWindow*3, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
