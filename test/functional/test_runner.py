...
def main():
    ...
    tmpdir = "%s/thincoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and thincoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "thincoind"]) is not None:
            print("%sWARNING!%s There is already a thincoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "THINCOIND" not in os.environ:
        os.environ["THINCOIND"] = build_dir + '/src/thincoind' + exeext
        os.environ["THINCOINCLI"] = build_dir + '/src/thincoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `thincoin-cli help` (`rpc_interface.txt`).
